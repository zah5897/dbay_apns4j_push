...
import com.zhan.app.service.DeviceService;
import com.zhan.app.util.SpringContextUtil;
...
public class ApnsConnectionImpl implements IApnsConnection {
	...
	// invalidate token error;
	private Queue<String> errorToken = new LinkedList<String>();
	...
	public ApnsConnectionImpl(SocketFactory factory, String host, int port, int maxRetries, int maxCacheLength,
			String name, String connName, int intervalTime, int timeout) {
	...
	public void sendNotification(PushNotification notification) {
		...
		synchronized (lock) {
			...
			if (!isSuccessful) {
				...
				// logger.info(String.format("%s Send success. count: %s,
				// notificaion: %s", connName,
				...
				try {
					...
					if (size == res.length && command == Command.ERROR) {
						...
						if (logger.isInfoEnabled()) {
							logger.info(String.format("%s Received error response. status: %s, id: %s, error-desc: %s",
						...
						synchronized (lock) {
							...
							while (!notificationCachedQueue.isEmpty()) {
								...
								if (pn.getId() == errorId) {
									errorToken.offer(pn.getToken());
									startHandleErrorToken();
									found = true;
								} else {
									/**
									 ...
									 * /CommunicatingWIthAPS.html As the
									 * document said, add the notifications
							...
							if (!found) {
								logger.warn(
										connName + " Didn't find error-notification in the queue. Maybe it's time to adjust cache length. id: "
												+ errorId);
							}
						}
	...
	boolean startHandleErrorToken = false;

	private void startHandleErrorToken() {
		if (startHandleErrorToken) {
			return;
		}
		startHandleErrorToken = true;

		new Thread() {
			public void run() {
				while (startHandleErrorToken) {
					String errorTokenStr = errorToken.poll();
					if (TextUtils.isEmpty(errorTokenStr)) {
						startHandleErrorToken = false;
						break;
					} else {
						try {
							DeviceService deviceService = SpringContextUtil.getBean("deviceService");
							if (deviceService != null) {
								deviceService.removeToken(errorTokenStr);
							} else {
